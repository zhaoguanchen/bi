package com.yiche.bigdata.solr;
...
public class SolrDataProvider extends DataProvider implements Aggregatable, Initializing {
    ...
    @DatasourceParameter(label = "{{'DATAPROVIDER.SOLR.SOLR_SERVERS'|translate}}", required = true, placeholder =
            "<ip>:<port>,[<ip>:<port>]...", type = DatasourceParameter.Type.Input, order = 1)
    private String SOLR_SERVERS = "solrServers";

    @DatasourceParameter(label = "{{'DATAPROVIDER.POOLEDCONNECTION'|translate}}", type = DatasourceParameter.Type
            .Checkbox, order = 2)
    private String POOLED = "pooled";

    @DatasourceParameter(label = "{{'DATAPROVIDER.AGGREGATABLE_PROVIDER_SOLR'|translate}}", type =
            DatasourceParameter.Type.Checkbox, order = 3)
    private String AGGREGATE_PROVIDER = "aggregateProvider";

    @QueryParameter(label = "{{'DATAPROVIDER.SOLR.COLLECTION'|translate}}", required = true, pageType = "test," +
            "dataset,widget", type = QueryParameter.Type.Input, order = 1)
    private String COLLECTION = "collection";

    @QueryParameter(label = "{{'DATAPROVIDER.SOLR.Q'|translate}}", required = true, pageType = "dataset,widget",
            value = "*:*", placeholder = "*:*|<fieldName>:<fieldValue>[ <AND|OR> <fieldName>:<fieldValue>]...", type
            = QueryParameter.Type.TextArea2, order = 2)
    private String Q = "q";

    @QueryParameter(label = "{{'DATAPROVIDER.SOLR.FQ'|translate}}", pageType = "dataset,widget", placeholder =
            "<fieldName>:<fieldValue>[,<fieldName>:<fieldValue>]...", type = QueryParameter.Type.Input, order = 3)
    private String FQ = "fq";

    @QueryParameter(label = "{{'DATAPROVIDER.SOLR.SORT'|translate}}", pageType = "dataset,widget", placeholder =
            "<fieldName> <ASC|DESC>[,<fieldName> <ASC|DESC>]...", type = QueryParameter.Type.Input, order = 4)
    private String SORT = "sort";

    @QueryParameter(label = "{{'DATAPROVIDER.SOLR.START'|translate}}", required = true, pageType = "dataset,widget",
            value = "0", placeholder = "default value is 0", type = QueryParameter.Type.Number, order = 5)
    private String START = "start";

    @QueryParameter(label = "{{'DATAPROVIDER.SOLR.ROWS'|translate}}", required = true, pageType = "dataset,widget",
            value = "10", placeholder = "default value is 10", type = QueryParameter.Type.Number, order = 6)
    private String ROWS = "rows";

    @QueryParameter(label = "{{'DATAPROVIDER.SOLR.FL'|translate}}", pageType = "dataset,widget", placeholder =
            "*|<fieldName>[,<fieldName>]...", type = QueryParameter.Type.Input, order = 7)
    ...
    private String getPoolKey(String solrServers, String collectionName) {
        return Hashing.md5().newHasher().putString(solrServers + "_" + collectionName, Charsets.UTF_8).hash()
                .toString();
    }
...
//        for (int i = 0; i < Arrays.stream(data).parallel().skip(1).filter(e -> e != null).map(e -> e[fi]).distinct
//                ().count(); i++) {
//            String kv = Arrays.stream(data).parallel().skip(1).filter(e -> e != null).map(e -> e[fi]).distinct()
//                    .iterator().next().trim();
//            PairData pairData = new PairData(kv, kv);
//            result.add(pairData);
//        }
//        return result;
    ...
    public AggregateResult queryAggData(AggConfig config) throws Exception {
        ...
        String kpi_type = "";
        //获取统计集合
        QueryResponse response = getQueryResponse(dataSource.get("solrServers"), query.get("collection"), getAggQuery
                (config));
        //获取行列字段
        Supplier<Stream<DimensionConfig>> dimStream = () -> Stream.concat(config.getColumns().stream(), config
                .getRows().stream());
        String dimColsStr = assembleDimColumns(dimStream.get()).replaceAll(" ", "");
        List<ColumnIndex> dimensionList = dimStream.get().map(ColumnIndex::fromDimensionConfig).collect(Collectors
                .toList());
        //没有聚合参数直接返回
        if (config.getValues().size() <= 0) {
            ...
            return new AggregateResult(dimensionList, strings, kpi_type);
        }
        //解析统计集合
        SimpleOrderedMap obj = (SimpleOrderedMap) ((SimpleOrderedMap) response.getResponse().get("facet_counts")).get
                ("facet_pivot");
        ...
        return new AggregateResult(dimensionList, result, kpi_type);
    }
        ...
        for (Object e : statList) {
            ...
            for (int x = 0; x < fileds.length; x++) {
                if (x <= fileds.length - config.getValues().size() - 1) {
                    ...
                    if (x == fileds.length - config.getValues().size() - 1) {
                        orderedMap = (SimpleOrderedMap) ((SimpleOrderedMap) orderedMap.get("stats")).get
                                ("stats_fields");
                    } else {
    ...
    private String configComponentToSql(ConfigComponent cc) {
        if (cc instanceof DimensionConfig) {
            ...
            String sql = compositeConfig.getConfigComponents().stream().map(e -> separateNull(e)).map(e ->
                    configComponentToSql(e)).collect(Collectors.joining(" " + compositeConfig.getType() + " "));
            return "(" + sql + ")";
        }
        return null;
    }
    ...
    private Function<DimensionConfig, String> filter2SolrCondtion = (config) -> {
        ...
        switch (config.getFilterType()) {
            case "=":
            case "eq":
                return config.getColumnName() + ":(" + IntStream.range(0, config.getValues().size()).boxed().map(i ->
                        config.getValues().get(i)).collect(Collectors.joining("OR", " ", " ")) + ")";
            case "≠":
            case "ne":
            ...
            case "(a,b]":
                if (config.getValues().size() >= 2) {
                    return config.getColumnName() + ":{" + config.getValues().get(0) + " TO " + config.getValues()
                            .get(1) + "]";
                } else {
                    return null;
                }
