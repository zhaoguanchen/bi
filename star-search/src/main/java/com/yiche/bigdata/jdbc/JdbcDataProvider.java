package com.yiche.bigdata.jdbc;
...
import com.yiche.bigdata.dataprovider.util.DPCommonUtils;
import com.yiche.bigdata.dataprovider.util.SqlHelper;
...
public class JdbcDataProvider extends DataProvider implements Aggregatable, Initializing {
    ...
    private static final CacheManager<Map<String, String>> typeCahceString = new HeapCacheManager<>();
        ...
        if (result != null) {
            ...
            try (
                ...
                for (int i = 0; i < columnCount; i++) {
                    ...
                    String a =metaData.getColumnTypeName(i+1);
                    System.out.print(a);
                }
    ...
    public Map<String, String> getColumnsAndType() throws Exception {
        ...
        String key = getLockKey();
        result = typeCahceString.get(key);
        if (result != null) {
            return result;
        } else {
            try (
                ...
                result = new HashedMap();
                for (int i = 0; i < columnCount; i++) {
                    result.put(metaData.getColumnLabel(i + 1).toUpperCase(), metaData.getColumnTypeName(i + 1));
//                    String a =metaData.getColumnTypeName(i+1);
//                    System.out.print(a);
                }
                typeCahceString.put(key, result, 12 * 60 * 60 * 1000);
                return result;
            }
        }
    }
    ...
    public Map<String, String> getColumnTypes() throws Exception {
        Map<String, Integer> columnTypes = getColumnType();
        Map<String, String> columnType = new HashedMap();
        for (String key : columnTypes.keySet()) {
            columnType.put(key, columnTypes.get(key).toString());
        }
        return columnType;
    }
