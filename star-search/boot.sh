...
APP_MAIN=com.yiche.bigdata.StarSearchApplication

#JVM启动参数
#-server:一定要作为第一个参数,在多个CPU时性能佳
#-Xloggc:记录GC日志,这里建议写成绝对路径,如此便可在任意目录下执行该shell脚本
JAVA_OPTS="-Duser.timezone=GMT+8 -server -Xmx2g -Xmx2g -XX:SurvivorRatio=4 -XX:+DisableExplicitGC -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=70 -XX:+PrintGCDetails -XX:+PrintGCDateStamps -Xloggc:$GC_LOGS/gc.log -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=$GC_LOGS -XX:ErrorFile=$GC_LOGS/hs_err_pid%p.log"
#JAVA_OPTS=$JAVA_OPTS' -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=50050 '
#JVM类加载路径
...
SERVICE_NAME="StarSearchApplication"
